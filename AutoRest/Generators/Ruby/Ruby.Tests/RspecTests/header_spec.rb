...
module HeaderModule
  ...
  describe 'Header' do
    ...
    it 'should post valid param dateTime' do
      result = @header_client.param_datetime('valid', DateTime.new(2010, 1, 1, 12, 34, 56, 'Z')).value!
      expect(result.response.status).to eq(200)
    end

    it 'should post min param dateTime' do
      result = @header_client.param_datetime('min', DateTime.new(1, 1, 1, 0, 0, 0, 'Z')).value!
      expect(result.response.status).to eq(200)
    end
    ...
    it 'should post valid byte' do
      result = @header_client.param_byte('valid', '啊齄丂狛狜隣郎隣兀﨩'.bytes).value!
      expect(result.response.status).to eq(200)
    end
    ...
    it 'should get response valid byte' do
      ...
      expect(result.response.status).to eq(200)
      expect(Base64.strict_decode64(result.response['value']).unpack('C*')).to eq('啊齄丂狛狜隣郎隣兀﨩'.bytes)
    end
  end
